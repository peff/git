...
static int add(int argc, const char **argv, const char *prefix)
{
	...
	if (master) {
		...
		if (refs_update_symref(get_main_ref_store(the_repository), buf.buf, buf2.buf, "remote add", 0))
...
static int mv(int argc, const char **argv, const char *prefix)
{
	...
	for (i = 0; i < remote_branches.nr; i++) {
		...
		strbuf_addf(&buf3, "remote: renamed %s to %s",
				item->string, buf.buf);
		if (refs_update_symref(get_main_ref_store(the_repository),
				       buf.buf, buf2.buf, buf3.buf, 0))
...
static int set_head(int argc, const char **argv, const char *prefix)
{
	...
	if (head_name) {
		...
		else if (refs_update_symref(get_main_ref_store(the_repository),
					    buf.buf, buf2.buf, "remote set-head", 0))
