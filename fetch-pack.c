...
static void mark_alternate_complete(struct fetch_negotiator *UNUSED(negotiator),
				    struct object *obj)
{
		    ...
		    struct ref **sought, int nr_sought)
{
	...
	if (args->keep_pack && (args->lock_pack || unpack_limit)) {
	...
	if (args->keep_pack || args->from_promisor)
		; /* never --unpack */
	...
	if (args->keep_pack && pack_lockfiles) {
