				       ...
				       enum object_type type)
{
	...
	/*
	 * We can use the type-bitmap to work in whole words
	...
	/*
	 * Clear any objects that weren't in the packfile (and so would not
	 * have been caught by the loop above. We'll have to check them
	 * individually.
	 */
	for (i = 0; i < eindex->count; i++) {
...
/*
 * NEEDSWORK We probably should take a bitmap_index pointer in functions that
 * call this, but the whole point of them is to transparently speed things up
 * when bitmaps are present. So we want to amortize the cost of loading the
 * bitmaps across many calls without having to pass bitmap_git through the call
 * stack.
 */
static struct bitmap_index *reusable_bitmap_git(void)
{
...
int bitmap_ahead_behind(struct commit *tip, struct commit *base, int *ahead, int *behind)
{
	...
	struct bitmap_index *bitmap_git = reusable_bitmap_git();
